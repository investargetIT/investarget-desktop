...
import { getConstant } from '../utils/util'

const routeNeedsTag = ["/register", URI_3, URI_6]
const routeNeedsIndustry = [URI_3]
...
export default {
  ...
  state: {
    ...
    countries: [],
    titles: [],
    currency: getConstant('currency', window.LANG),
    audit: getConstant('audit', window.LANG),
    industries: [],
    ...
    organizationTypes: [{id: 1,name: "基金"},{id: 2,name: "律所"},{id: 3,name: "投行"},{id: 4,name: "会计师事务所"},{id: 5,name: "咨询"},{id: 6,name: "证券"},{id: 7,name: "银行"},{id: 8,name: "信托"},{id: 9,name: "租赁"},{id: 10,name: "保险"},{id: 11,name: "期货"},{id: 12,name: "上市公司"},{id: 13,name: "新三板上市公司"},{id: 14,name: "非上市公司"},{id: 15,name: "政府引导性基金"},{id: 16,name: "金融机构直投基金"},{id: 17,name: "上市公司产业基金"},{id: 18,name: "其他"},{id: 19,name: "个人"}],
  },
  reducers: {
    ...
    saveIndustries(state, { payload: industries }) {
  ...
  effects: {
    ...
    *requestIndustries({}, { call, put }) {
      const { data } = yield call(api.getIndustries)
      yield put({ type: 'saveIndustries', payload: data })
    },
    *getIndustries({}, { put, select }) {
      const industries = yield select(state => state.app.industries)
      industries.length > 0 || (yield put({ type: 'requestIndustries' }))
    },
  },
  subscriptions: {
    setup({ history, dispatch }) {
      return history.listen(({ pathname }) => {
        ...
        if (routeNeedsIndustry.includes(pathname)) {
          dispatch({ type: 'getIndustries' })
        }
