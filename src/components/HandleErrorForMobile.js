...
class HandleErrorForMobile extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.error) {
      ...
      this.props.dispatch({ type: 'app/dismissErrorForMobile' })
    }
  }
  ...
  handleApiError(code, msg, detail) {
    ...
    switch (code) {
      case 3000:
        window.echo('mobile handle error');
        if (!HandleErrorForMobile.handleSessionExpiration) {
          HandleErrorForMobile.handleSessionExpiration = true
          Modal.error({
...
function mapStateToProps(state) {
  const { errorForMobile } = state.app
  return { error: errorForMobile }
}

export default connect(mapStateToProps)(HandleErrorForMobile)
